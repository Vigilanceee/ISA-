...
The reference run used kernel SHA-256
`9f01869f9afde1e80fc13d162826e068cf44c41bb5e560e78355a34e45d06102`.
The exact kernel retains the reference implementation behind
`raw_kernel_backend=legacy`, so the benchmark remains reproducible alongside
the split-reduction implementation.
...
optimized exact backend; both defaults remain `legacy` so the reference run
cannot silently change.

Generated profiler traces and benchmark outputs are written below the selected
`OUTPUT_DIR` and are intentionally excluded from source control.
