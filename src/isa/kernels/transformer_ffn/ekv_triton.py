...
if triton is not None:
    ...
    @triton.jit
    def _ekv_current(v, vth, inv_2nut: tl.constexpr, nvd: tl.constexpr, i_s: tl.constexpr, v_sat: tl.constexpr):
        u1 = (v - vth) * inv_2nut
        u2 = (v - vth - nvd) * inv_2nut
        sp1 = _softplus(u1)
        sp2 = _softplus(u2)
    ...
    @triton.jit
    def _ekv_grad_v(v, vth, inv_2nut: tl.constexpr, nvd: tl.constexpr, i_s: tl.constexpr, v_sat: tl.constexpr):
        u1 = (v - vth) * inv_2nut
        u2 = (v - vth - nvd) * inv_2nut
        sp1 = _softplus(u1)
        sp2 = _softplus(u2)
    ...
    @triton.jit
    def _ekv_grad_vth(v, vth, inv_2nut: tl.constexpr, nvd: tl.constexpr, i_s: tl.constexpr, v_sat: tl.constexpr):
        u1 = (v - vth) * inv_2nut
        u2 = (v - vth - nvd) * inv_2nut
        sp1 = _softplus(u1)
        sp2 = _softplus(u2)
    ...
    def _forward_kernel(
        ...
        inv_2nut: tl.constexpr, nvd: tl.constexpr, i_s: tl.constexpr, v_sat: tl.constexpr,
        ...
        for k0 in range(0, K, BLOCK_K):
            ...
            ip = _ekv_current(xv[:, None, :], wp[None, :, :], inv_2nut, nvd, i_s, v_sat)
            inn = _ekv_current(xv[:, None, :], wn[None, :, :], inv_2nut, nvd, i_s, v_sat)
    ...
    def _grad_x_kernel(
        ...
        for o0 in range(0, O, BLOCK_O):
            ...
            dpos = _ekv_grad_v(xv[:, None, :], wp[None, :, :], inv_2nut, nvd, i_s, v_sat)
            dneg = _ekv_grad_v(xv[:, None, :], wn[None, :, :], inv_2nut, nvd, i_s, v_sat)
    ...
    def _grad_w_kernel(
        ...
        for m0 in range(0, M, BLOCK_M):
            ...
            dwp = _ekv_grad_vth(xv[None, :, :], wp[:, None, :], inv_2nut, nvd, i_s, v_sat)
            dwn = _ekv_grad_vth(xv[None, :, :], wn[:, None, :], inv_2nut, nvd, i_s, v_sat)
